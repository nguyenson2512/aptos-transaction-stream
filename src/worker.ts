...
export class Worker {
  ...
  async run({ perf }: { perf?: number } = {}) {
    ...
    //avoid Error: INTERNAL: Received RST_STREAM with code 0
    const nextVersionToProcess = await this.dataSource
      .getRepository(NextVersionToProcess)
      .find();
    const startingVersion = BigInt(
      nextVersionToProcess?.[0]?.nextVersion ||
        this.config.starting_version ||
        0n,
    );
      ...
      async (response: aptos.indexer.v1.TransactionsResponse) => {
        ...
        if (numProcessed) {
          await this.dataSource.transaction(async (txnManager) => {
            ...
            console.log("NumProcessed: ", nextVersionToProcess);
          ...
          console.log("Checkpoint: ", nextVersionToProcess);
